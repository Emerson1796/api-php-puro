...
class EnderecoRepository {
    ...
    public function findByUsuario($id_usuario) {
        $sql = 'SELECT * FROM enderecos WHERE id_usuario = ? AND deleted_at IS NULL';
        $stmt = $this->conexao->prepare($sql);
    ...
    public function save(Endereco $endereco) {
        ...
        $sql = 'INSERT INTO enderecos (id_usuario, logradouro, numero, complemento, cep, id_cidade, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())';
        $stmt = $this->conexao->prepare($sql);
        $stmt->execute([
    ...
    private function update(Endereco $endereco) {
        $sql = 'UPDATE enderecos SET id_usuario = ?, logradouro = ?, numero = ?, complemento = ?, cep = ?, id_cidade = ?, updated_at = NOW() WHERE id = ?';
        $stmt = $this->conexao->prepare($sql);
        return $stmt->execute([
            $endereco->getIdUsuario(),
            $endereco->getLogradouro(),
